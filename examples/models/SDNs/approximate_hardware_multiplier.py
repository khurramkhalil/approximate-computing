...
# Approximate multiplier function
def approximate_multiply(a, b):
    # result = a * b
    # error = torch.randn_like(result) * 0.05  # Add small random error
    result = mul8s_1L2L.mul(a, b)
    result = torch.tensor([u2s(v) for v in result])
    return result

# Approximate Conv2d layer
class ApproximateConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
        super(ApproximateConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)

    def forward(self, input):
        return F.conv2d(input, approximate_multiply(self.weight, 1), self.bias, self.stride, self.padding, self.dilation, self.groups)
...
class wide_basic(nn.Module):
    def __init__(self, in_channels, channels, dropout_rate, params, stride=1):
        ...
        conv_layer.append(ApproximateConv2d(in_channels, channels, kernel_size=3, padding=1, bias=True))
        conv_layer.append(nn.Dropout(p=dropout_rate))
        conv_layer.append(nn.BatchNorm2d(channels))
        conv_layer.append(nn.ReLU(inplace=True))
        conv_layer.append(ApproximateConv2d(channels, channels, kernel_size=3, stride=stride, padding=1, bias=True))
        self.layers.append(nn.Sequential(*conv_layer))

        shortcut = nn.Sequential()
        if stride != 1 or in_channels != channels:
            shortcut = nn.Sequential(
                ApproximateConv2d(in_channels, channels, kernel_size=1, stride=stride, bias=True),
            )
...
class WideResNet_SDN(nn.Module):
    def __init__(self,  num_classes=10):
        ...
        if self.input_size == 32:  # cifar10 and cifar100
            ...
            self.init_conv = ApproximateConv2d(3, self.in_channels, kernel_size=3, stride=1, padding=1, bias=True)
        elif self.input_size == 64:  # tiny imagenet
            self.cur_input_size = int(self.input_size/2)
            self.init_conv = ApproximateConv2d(3, self.in_channels, kernel_size=3, stride=2, padding=1, bias=True)
    ...
    def initialize_weights(self):
        for m in self.modules():
            if isinstance(m, ApproximateConv2d):
...
def wideresnet_sdn_v1(pretrained=False, progress=True, device="cpu", axx_mult = 'mul8s_acc', **kwargs):
...
# Function to replace all Conv2d layers with ApproximateConv2d
def replace_conv_layers(model):
    for name, module in model.named_children():
        if isinstance(module, nn.Conv2d):
            setattr(model, name, ApproximateConv2d(module.in_channels, module.out_channels, 
...
# Test the approximate model
def test_approximate_model():
    params = {
        'num_blocks': [3, 3, 3],
        'widen_factor': 2,
        'num_classes': 10,
        ...
        'input_size': 32,
        'add_ic': [[0, 1, 0], [1, 0, 1], [0, 1, 0]],
        'init_weights': True
    }
    params = {'num_classes': 10}

    model = WideResNet_SDN(num_classes=10)
